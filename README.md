...
Демонстрационный проект микросервисной архитектуры на Go: «завод по сборке космических кораблей».
...
**Архитектура кода.** Слои `API → Service → Repository → Client` с раздельными моделями
(API / доменная / хранилища) и конвертерами между ними. Инверсия зависимостей: интерфейсы
объявлены на стороне потребителя (`deps.go`), реализации подставляются ручным DI-контейнером
с ленивой инициализацией и закрытием ресурсов в обратном порядке.

**DDD.** Сущности с поведением, а не анемичные структуры: `Reserve` / `Release` / `Commit`
не дают детали уйти в невалидное состояние. Value objects для характеристик деталей,
агрегат «заказ + позиции» с единой точкой входа, доменный сервис проверки совместимости корпуса,
двигателя, щита и оружия. Доменный слой не знает ни про БД, ни про логгер.

**Данные и согласованность.** PostgreSQL через pgx с пулом соединений, запросы на Squirrel,
миграции goose. Transaction manager прячет транзакцию в контекст — репозиторий не знает,
что работает внутри неё. Конкурентное резервирование деталей защищено `SELECT ... FOR UPDATE`
с детерминированным порядком блокировок (иначе — deadlock), характеристики разных типов деталей
лежат в JSONB без раздувания схемы.

**Асинхронность.** Kafka (KRaft) как шина событий: `order.paid` → сборка → `assembly.ship-assembled`.
Consumer groups для горизонтального масштабирования, идемпотентная обработка — повторная доставка
не ломает данные.

**Аутентификация.** Пароли через bcrypt, сессии в Redis с TTL, gRPC-интерцептор и HTTP middleware
для проверки сессии, проброс идентификатора пользователя по цепочке вызовов через gRPC metadata.

**Отказоустойчивость.** Распределённый rate limiter на Redis — единый лимит на все реплики,
при недоступности Redis запросы пропускаются, а не блокируются. Graceful shutdown, healthcheck'и,
graceful degradation телеметрии: упавший Elasticsearch не роняет сервис.

**Наблюдаемость.** OpenTelemetry сквозь все сервисы: структурированные логи (slog) одновременно
в stdout и Elasticsearch, метрики в Prometheus с бизнес-дашбордами в Grafana, трейсы в Jaeger.
Один `trace_id` связывает записи всех сервисов — от создания заказа до сборки корабля.

**Тесты и эксплуатация.** Табличные unit-тесты с моками (mockery) и параллельным запуском,
API-тесты, тесты конкурентности и rate limiter'а, e2e с реальной Kafka через testcontainers.
Нагрузочное тестирование vegeta. Вся система — от инфраструктуры до пяти сервисов за Nginx —
поднимается одной командой; сборка образов многоэтапная, с BuildKit-кешем.
...
├── docs/              # технические заметки: почему сделано именно так
├── deploy/
...
Неочевидные технические решения вынесены в [docs/](docs/README.md): почему транзакция живёт
в контексте, зачем `ORDER BY` перед `FOR UPDATE`, как session-uuid проходит через Kafka,
что сломается без index template в Elasticsearch и почему нагрузочный тест бьёт по нескольким
парам деталей.
