...
func (r *repository) GetByUUID(ctx context.Context, userUUID uuid.UUID) (model.User, error) {
	const queryOrder = `SELECT uuid, login, password_hash, created_at, updated_at FROM users WHERE uuid = $1`

	var user record.User

	err := r.getter.DefaultTrOrDB(ctx, r.pool).QueryRow(ctx, queryOrder, userUUID).Scan(
